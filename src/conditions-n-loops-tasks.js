...
function rotateMatrix(matrix) {
  ...
  for (let i = 0; i < sizeMatrix; i += 1) {
    for (let j = i + 1; j < sizeMatrix; j += 1) {
  ...
  for (let i = 0; i < sizeMatrix; i += 1) {
    for (let j = 0; j < sizeMatrix / 2; j += 1) {
      const temp = newMatrix[i][j];
      newMatrix[i][j] = newMatrix[i][sizeMatrix - 1 - j];
      newMatrix[i][sizeMatrix - 1 - j] = temp;
    }
  }
  return newMatrix;
}
...
function getNearestBigger(number) {
  ...
  let pivotIndex = digits.length - 1;
  while (pivotIndex > 0 && digits[pivotIndex - 1] >= digits[pivotIndex]) {
    pivotIndex -= 1;
  }
  if (pivotIndex === 0) {
    return number;
  ...
  while (digits[swapIndex] <= digits[pivotIndex - 1]) {
    swapIndex -= 1;
  }
  [digits[swapIndex], digits[pivotIndex - 1]] = [
    digits[pivotIndex - 1],
    digits[swapIndex],
  ];
  const sortArr = digits.splice(pivotIndex).sort();
  const nextGreaterNum = digits.join('') + sortArr.join('');
  return Number(nextGreaterNum);
}
